...
esp_err_t i2c_write(uint8_t address, uint8_t size, uint8_t* data){
   ...
   if (ret == ESP_FAIL) {
      if(LOGE_I2C) ESP_LOGE(TAG,"Unable to write (address %02x).", address);
      if(LOGE_I2C) ESP_LOGE(TAG,"description: %s", esp_err_to_name(ret));
   }
   else
      if(LOGI_I2C) ESP_LOGI(TAG, "Write OK (address %02x).", address);
...
esp_err_t i2c_read(uint8_t address, uint8_t reg, uint8_t size, uint8_t *data) {
   ...
   i2c_master_write_byte(cmd, reg, ACK);
	...
	i2c_cmd_link_delete(cmd);
	if( ret == ESP_FAIL ) {
	   if(LOGE_I2C) ESP_LOGE(TAG,"Unable to write (address %02x, reg %02x",address, reg);
		if(LOGE_I2C) ESP_LOGE(TAG,"Description: %s", esp_err_to_name(ret));
	}
	//read byte
	...
	i2c_cmd_link_delete(cmd);
	if(ret == ESP_FAIL) {
	   if(LOGE_I2C) ESP_LOGE(TAG,"Unable to read (address %02x, reg %02x",address, reg);
		if(LOGE_I2C) ESP_LOGE(TAG,"Description: %s", esp_err_to_name(ret));
	}
   else
      if(LOGI_I2C) ESP_LOGI(TAG,"Read OK (address %02x, reg %02x).", address, reg);
   return ret;
}