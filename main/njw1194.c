...
esp_err_t njw_write (uint8_t chip_address, uint8_t select_address, uint8_t data){
	if(LOGI_NJW) ESP_LOGI(TAG, "NJW1194 write.");
	uint8_t spi_data[2]={0,0};
	spi_data[1]=chip_address|(select_address<<4);
	spi_data[0]=data;
	esp_err_t ret =spi_write(2*8, spi_data);
	if (ret!=ESP_OK) {
		if(LOGE_NJW) ESP_LOGE(TAG, "NJW1194 write failed: address %02x, data %02x.", spi_data[0], spi_data[1]);
   	}
   	else
      	if(LOGI_NJW) ESP_LOGI(TAG, "NJW1194 write OK: address %02x, data %02x.", spi_data[0], spi_data[1]);
   	return ret;
}
